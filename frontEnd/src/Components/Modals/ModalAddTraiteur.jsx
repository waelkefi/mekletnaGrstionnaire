...
const ModalAddTraiteur = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (values) => {
    ...
    dispatch(addTraiteur(clientData))
      .then(result => {
        if (result) {
          successAdd();
        }
  ...
  return (
    <div className={`modal ${isOpen ? 'open' : ''}`} onClick={handleOverlayClick}>
      {modalOpen &&
        <ModalAnimation isOpen={modalOpen} onClose={closeModal} message="Traiteur ajouté avec succès" />
      }
      <Snackbar
...
export default ModalAddTraiteur;